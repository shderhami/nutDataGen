...
PIPELINE_VERSION = "cv-v8.7"  # v8.7: SUPPLEMENT_CATEGORIES += 'Animal Fat' (new schema category for terrestrial rendered fats; tallow 10064 recategorized out of Fish Oil to stop the formulator's fish-oil PUFA advisory - delivered-floor CV treatment deliberately PRESERVED, so this bump changes no CV mechanics, only the category key). v8.6: FOOD_MAP + FDC_MAP grew (10057 green beans -> FCDB 764; 8 obs from older Danish composition surveys; Cu/Zn EXCLUDED as 1977-80 contaminant monitoring per pork precedent; B2 workbook range corrupt min>max, self-drops). v8.5: FOOD_MAP + FDC_MAP grew (10056 zucchini -> FCDB 56 "Squash, raw" = Danish courgette; 19 eligible n=8 obs from study 2127, biotin excluded as the cell's own literature_range stats). NOTE: single-food cv_assign runs can trip the SE-vs-range median gate on watery produce (FND zero-min censored ranges: zucchini fat 0.0-0.42, Fe 0.0-0.396) - verified by hand 2026-08-31, not a parse bug; whole-table runs carry the gate as designed. v8.4: completes the v8.3 reassignment - v8.3 left BOTH 10035 and 10055 holding FCDB 1019 in FOOD_MAP, and build()'s dict inversion kept 10035 (7 obs, excludes retired, none to 10055); the stray 10035 line is now removed so 1019's 7 obs land on the frame-matched 10055. v8.3: FCDB 1019 chuck reassigned 10035 -> 10055 (trimmed chuck roll, fat 13.1~13.9; fat/water/ash excludes retired); FDC_MAP 10055:(170814,). v8.2: docs-only cv_intl FOOD_MAP note extended (10054 skinless breast unmapped, same whole-bird reasoning as 10048); no behavior change.  # v8.1: FOOD_MAP + FDC_MAP grew (10050 beef heart -> FCDB 641; 156 obs, +3 range cells); sigma2 recheck 0.243 vs applied 0.240 (within tolerance, constant unchanged). v8: international same-food CV pooling at discounted effective n (+ v7 literature_range tier)
...
SUPPLEMENT_CATEGORIES = {"Supplement", "Fish Oil", "Animal Fat"}
